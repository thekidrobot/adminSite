...
$EW_REPORT_TABLE_SQL_SELECT = "SELECT log_descargas.fecha_descarga, log_descargas.fecha_descarga As fechadescarga, archivos.titulo, usuarios.NombreCompleto, grupos.grupos FROM " . $EW_REPORT_TABLE_SQL_FROM;
...
$sv1_fechadescarga = ""; $sv1d_fechadescarga = "";
$sv2_fechadescarga = ""; $sv2d_fechadescarga = "";
$so1_fechadescarga = ""; $so1d_fechadescarga = "";
$so2_fechadescarga = ""; $so2d_fechadescarga = "";
$sc_fechadescarga = ""; $scd_fechadescarga = "";
...
$o_NombreCompleto = NULL; $t_NombreCompleto = NULL; $ft_NombreCompleto = 200; $rf_NombreCompleto = NULL; $rt_NombreCompleto = NULL;
...
var elm = form_obj.sv1_fechadescarga;
if (elm && !ewrpt_CheckEuroDate(elm.value)) {
	if (!ewrpt_OnError(elm, "Incorrect date, format = dd/mm/yyyy - Fechadescarga"))
		return false;
}
var elm = form_obj.sv2_fechadescarga;
if (elm && !ewrpt_CheckEuroDate(elm.value)) {
	if (!ewrpt_OnError(elm, "Incorrect date, format = dd/mm/yyyy - Fechadescarga"))
		return false;
}
...
ewrpt_CreatePopup("Report1_fecha_descarga", [<?php echo $jsdata ?>]);
<?php $jsdata = ewrpt_GetJsData($val_titulo, $sel_titulo, $ft_titulo) ?>
ewrpt_CreatePopup("Report1_titulo", [<?php echo $jsdata ?>]);
</script>
<div id="Report1_fecha_descarga_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
<div id="Report1_titulo_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
...
&nbsp;&nbsp;<a href="Report1smry.php?cmd=reset">Reset All Filters</a>
<?php } ?>
<br /><br />
</div></td></tr>
<!-- Top Container (End) -->
...
<!-- Search form (begin) -->
<form name="fReport1summaryfilter" id="fReport1summaryfilter" action="Report1smry.php" class="ewForm" onSubmit="return ewrpt_ValidateExtFilter(this);">
<table class="ewRptExtFilter">
	<tr>
		<td><span class="phpreportmaker">Fechadescarga</span></td>
		<td><span class="ewRptSearchOpr">between<input type="hidden" name="so1_fechadescarga" id="so1_fechadescarga" value="BETWEEN"></span></td>
		<td>
			<table cellspacing="0" class="ewItemTable"><tr>
				<td><span class="phpreportmaker">
<input type="text" name="sv1_fechadescarga" id="sv1_fechadescarga" value="<?php echo ewrpt_HtmlEncode($sv1_fechadescarga) ?>"<?php echo ($sClearExtFilter == 'Report1_fechadescarga') ? " class=\"ewInputCleared\"" : "" ?>>
</span></td>
				<td><span class="ewRptSearchOpr" id="btw1_fechadescarga" name="btw1_fechadescarga">&nbsp;and&nbsp;</span></td>
				<td><span class="phpreportmaker" id="btw1_fechadescarga" name="btw1_fechadescarga">
<input type="text" name="sv2_fechadescarga" id="sv2_fechadescarga" value="<?php echo ewrpt_HtmlEncode($sv2_fechadescarga) ?>"<?php echo ($sClearExtFilter == 'Report1_fechadescarga') ? " class=\"ewInputCleared\"" : "" ?>>
</span></td>
			</tr></table>			
		</td>
	</tr>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td>Titulo</td>
			<td style="width: 20px;" align="right"><a href="#" onClick="ewrpt_ShowPopup(this.name, 'Report1_titulo', false, '<?php echo $rf_titulo; ?>', '<?php echo $rt_titulo; ?>');return false;" name="x_titulo<?php echo $cnt[0][0]; ?>" id="x_titulo<?php echo $cnt[0][0]; ?>"><img src="phprptimages/popup.gif" width="15" height="14" align="texttop" border="0" alt=""></a></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
function GetRow($opt) {
	...
	if (!$rs->EOF) {
		...
		$GLOBALS['x_fechadescarga'] = $rs->fields('fechadescarga');
		...
		$val[3] = $GLOBALS['x_NombreCompleto'];
		$val[4] = $GLOBALS['x_grupos'];
	} else {
		$GLOBALS['x_fecha_descarga'] = "";
		$GLOBALS['x_fechadescarga'] = "";
		$GLOBALS['x_titulo'] = "";
...
function SetupPopup() {
	...
	if (is_array(@$_SESSION["sel_Report1_fecha_descarga"])) {
		LoadSelectionFromSession('fecha_descarga');
	} elseif (@$_SESSION["sel_Report1_fecha_descarga"] == EW_REPORT_INIT_VALUE) { // Select all
		$GLOBALS["sel_fecha_descarga"] = "";
	}

	// Get Titulo selected values
	if (is_array(@$_SESSION["sel_Report1_titulo"])) {
		LoadSelectionFromSession('titulo');
	} elseif (@$_SESSION["sel_Report1_titulo"] == EW_REPORT_INIT_VALUE) { // Select all
		$GLOBALS["sel_titulo"] = "";
	}
}
...
function GetExtendedFilter() {
	...
	if ($bPostBack) {
		...
		// Clear extended filter for field fechadescarga
		if ($GLOBALS["sClearExtFilter"] == 'Report1_fechadescarga')
			SetSessionFilterValues('', '=', 'AND', '', '=', 'fechadescarga');
	...
	// Reset search command
		...
		SetSessionFilterValues($GLOBALS["sv1_fechadescarga"], $GLOBALS["so1_fechadescarga"], $GLOBALS["sc_fechadescarga"], $GLOBALS["sv2_fechadescarga"], $GLOBALS["so2_fechadescarga"], 'fechadescarga');
		$bSetupFilter = TRUE;
	} else {

		// Field fechadescarga
		if (GetFilterValues($GLOBALS["sv1_fechadescarga"], $GLOBALS["so1_fechadescarga"], $GLOBALS["sc_fechadescarga"], $GLOBALS["sv2_fechadescarga"], $GLOBALS["so2_fechadescarga"], 'fechadescarga')) {
			$bSetupFilter = TRUE;
	...
	if ($bRestoreSession) {
		...
		// Field fechadescarga
		GetSessionFilterValues($GLOBALS["sv1_fechadescarga"], $GLOBALS["so1_fechadescarga"], $GLOBALS["sc_fechadescarga"], $GLOBALS["sv2_fechadescarga"], $GLOBALS["so2_fechadescarga"], 'fechadescarga');
	}

	// Build SQL
	// Field fechadescarga

	BuildExtendedFilter($sFilter, 'fechadescarga', '`fechadescarga`', EW_REPORT_DATATYPE_DATE, 0, $GLOBALS["sv1_fechadescarga"], $GLOBALS["so1_fechadescarga"], $GLOBALS["sc_fechadescarga"], $GLOBALS["sv2_fechadescarga"], $GLOBALS["so2_fechadescarga"]);
	...
	if ($bSetupFilter) {
		...
		// Field fechadescarga
		//$GLOBALS["sel_fechadescarga"] = "";

		$sWrk = "";
		BuildExtendedFilter($sWrk, 'fechadescarga', '`fechadescarga`', EW_REPORT_DATATYPE_DATE, 0, $GLOBALS["sv1_fechadescarga"], $GLOBALS["so1_fechadescarga"], $GLOBALS["sc_fechadescarga"], $GLOBALS["sv2_fechadescarga"], $GLOBALS["so2_fechadescarga"]);
		LoadSelectionFromFilter('fechadescarga', $sWrk, $GLOBALS["sel_fechadescarga"]);
		$_SESSION['sel_Report1_fechadescarga'] = ($GLOBALS["sel_fechadescarga"] == "") ? EW_REPORT_INIT_VALUE : $GLOBALS["sel_fechadescarga"];
	}
	return $sFilter;
}
...
function GetSessionDropDownValue(&$sv, $parm) {
	GetSessionValue($sv, 'sv_Report1_' . $parm);
}

// Get filter values from session
function GetSessionFilterValues(&$sv1, &$so1, &$sc, &$sv2, &$so2, $parm) {
	GetSessionValue($sv1, 'sv1_Report1_' . $parm);
	GetSessionValue($so1, 'so1_Report1_' . $parm);
	GetSessionValue($sc, 'sc_Report1_' . $parm);
	GetSessionValue($sv2, 'sv2_Report1_' . $parm);
	GetSessionValue($so2, 'so2_Report1_' . $parm);
}
...
function LoadDefaultFilters() {
	...
	*/
	...
	// Field fechadescarga
	SetDefaultExtFilter('fechadescarga', 'BETWEEN', NULL, 'AND', '=', NULL);
	ApplyDefaultExtFilter('fechadescarga');

	//$GLOBALS["seld_fechadescarga"] = "";
	$sWrk = "";
	BuildExtendedFilter($sWrk, 'fechadescarga', '`fechadescarga`', EW_REPORT_DATATYPE_DATE, 0, $GLOBALS["sv1_fechadescarga"], $GLOBALS["so1_fechadescarga"], $GLOBALS["sc_fechadescarga"], $GLOBALS["sv2_fechadescarga"], $GLOBALS["so2_fechadescarga"]);
	LoadSelectionFromFilter('fechadescarga', $sWrk, $GLOBALS["seld_fechadescarga"]);
	$GLOBALS["sel_fechadescarga"] = $GLOBALS["seld_fechadescarga"];
	...
	*/
	...
	// $seld_fecha_descarga = array("val1", "val2");

	$GLOBALS["seld_fecha_descarga"] = "";
	$GLOBALS["sel_fecha_descarga"] =  $GLOBALS["seld_fecha_descarga"];
...
function CheckFilter() {
	...
	// Check fechadescarga text filter
	if (TextFilterApplied("fechadescarga"))
		return TRUE;

	// Check fechadescarga popup filter
	if (!ewrpt_MatchedArray($GLOBALS["seld_fechadescarga"], $GLOBALS["sel_fechadescarga"]))
		return TRUE;
...
function ShowFilterList() {
	...
	$sExtWrk = "";
	$sWrk = "";
	BuildExtendedFilter($sExtWrk, 'fechadescarga', '`fechadescarga`', EW_REPORT_DATATYPE_DATE, 0, $GLOBALS["sv1_fechadescarga"], $GLOBALS["so1_fechadescarga"], $GLOBALS["sc_fechadescarga"], $GLOBALS["sv2_fechadescarga"], $GLOBALS["so2_fechadescarga"]);
	if (is_array($GLOBALS["sel_fechadescarga"]))
		$sWrk = ewrpt_JoinArray($GLOBALS["sel_fechadescarga"], ", ", EW_REPORT_DATATYPE_DATE);
	if ($sExtWrk <> "" || $sWrk <> "")
		$sFilterList .= "Fechadescarga<br />";
	if ($sExtWrk <> "")
		$sFilterList .= "&nbsp;&nbsp;$sExtWrk<br />";
	if ($sWrk <> "")
		$sFilterList .= "&nbsp;&nbsp;$sWrk<br />";
...
function GetPopupFilter() {
	...
	if (!ExtendedFilterExist('fechadescarga', '`fechadescarga`', EW_REPORT_DATATYPE_DATE, 0, $GLOBALS["sv1_fechadescarga"], $GLOBALS["so1_fechadescarga"], $GLOBALS["sc_fechadescarga"], $GLOBALS["sv2_fechadescarga"], $GLOBALS["so2_fechadescarga"])) {
	if (is_array($GLOBALS["sel_fechadescarga"])) {
		if ($sWrk <> "") $sWrk .= " AND ";
		$sWrk .= ewrpt_FilterSQL($GLOBALS["sel_fechadescarga"], "`fechadescarga`", EW_REPORT_DATATYPE_DATE, $GLOBALS["af_fechadescarga"]);
	}
	}
	if (is_array($GLOBALS["sel_titulo"])) {
		if ($sWrk <> "") $sWrk .= " AND ";
...
function getSort()
{
	...
	if (strlen(@$_GET["cmd"]) > 0) {
		...
		if ($sCmd == "resetsort") {
			...
			$_SESSION["sort_Report1_fecha_descarga"] = "";
			$_SESSION["sort_Report1_titulo"] = "";
			$_SESSION["sort_Report1_NombreCompleto"] = "";
			$_SESSION["sort_Report1_grupos"] = "";
		}
	...
	if (@$_SESSION[EW_REPORT_TABLE_SESSION_ORDER_BY] == "") {
		@$_SESSION[EW_REPORT_TABLE_SESSION_ORDER_BY] = "log_descargas.fecha_descarga DESC";
		$_SESSION["sort_Report1_fecha_descarga"] = "DESC";
	}
