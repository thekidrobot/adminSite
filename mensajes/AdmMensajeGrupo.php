...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form2")) {
  $updateSQL = sprintf("UPDATE mensajes_grupo SET ID_GRUPO=%s, MENSAJE=%s, FECHA_VENCIMIENTO=%s WHERE ID_MENSAJE=%s",
                       GetSQLValueString($_POST['ID_GRUPO'], "int"),
                       GetSQLValueString($_POST['MENSAJE'], "text"),
                       GetSQLValueString($_POST['FECHA_VENCE'], "date"),
  ...
  $updateGoTo = "AdmMensajeGrupo.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_cnxRamp, $cnxRamp);
$query_rsMensaje = sprintf("SELECT * FROM mensajes_grupo WHERE ID_MENSAJE = %s", GetSQLValueString($colname_rsMensaje, "int"));
...
$sql='SELECT grupos.grupos, mensajes_grupo.ID_MENSAJE, mensajes_grupo.ID_GRUPO, mensajes_grupo.MENSAJE, mensajes_grupo.FECHA_CREACION, mensajes_grupo.FECHA_VENCIMIENTO
FROM mensajes_grupo INNER JOIN grupos ON mensajes_grupo.ID_GRUPO = grupos.IdGrupos';

$colname_rsMensajesTexto = "-1";
if (isset($_POST['texto'])) {
  $colname_rsMensajesTexto = $_POST['texto'];
  $donde = "WHERE mensajes_grupo.MENSAJE Like '%" . $colname_rsMensajesTexto . "%'";
}

$colname_rsMensajeusuario = "-1";
if (isset($_POST['usuario'])) {
  $colname_rsMensajeusuario = $_POST['usuario'];
  $donde = "WHERE grupos.grupos Like '%" . $colname_rsMensajeusuario . "%'";
}
...
<div id="apDiv1">
  <form id="form1" name="form1" method="post" action="">
    <table width="100%" border="0" class="contform">
      <tr>
        <td colspan="3" class="TituloAzul">Buscar por Grupo</td>
      </tr>
      <tr>
...
<div id="apDiv3">
  <table width="100%" border="0" cellpadding="0" cellspacing="1">
    <tr class="TituloAzul">
      ...
      <td width="19%">Fecha Vencimiento</td>
    </tr>
    <?php do { ?>
      <tr class="linea1">
        <td><a href="AdmMensajeGrupo.php?msg=<?php echo "" . $row_rsMensajesTexto['ID_MENSAJE']; ?>"><?php echo "" . $row_rsMensajesTexto['ID_MENSAJE']; ?></a></td>
        <td><?php echo "" . $row_rsMensajesTexto['grupos']; ?></td>
        <td><?php echo "" . substr($row_rsMensajesTexto['MENSAJE'],0,50) . '...'; ?></td>
        <td><?php echo "" . $row_rsMensajesTexto['FECHA_CREACION']; ?></td>
        <td><?php echo "" . $row_rsMensajesTexto['FECHA_VENCIMIENTO']; ?></td>
      </tr>
...
<form action="<?php echo $editFormAction; ?>" method="post" name="form2" id="form2">
    <table align="center" class="contform">
      ...
      <tr valign="baseline">
        ...
        <td><label>
<input name="FECHA_VENCE" type="text" class="textareas" id="FECHA_VENCE" value="<?php echo htmlentities($row_rsMensaje['FECHA_VENCIMIENTO'], ENT_COMPAT, 'utf-8'); ?>" />
        </label></td>
      </tr>
      <tr valign="baseline">
        <td nowrap="nowrap" align="right"><input name="ID_GRUPO" type="hidden" id="ID_GRUPO" value="<?php echo htmlentities($row_rsMensaje['ID_GRUPO'], ENT_COMPAT, 'utf-8'); ?>" size="32" /></td>
